...
# also get the 34 resonance field at a particular microwave frequency
# requires a finer grid of B around the resonance position to be accurate

t_frq = 3.629
t_frq_idx = np.where(transs[:,5,0] < t_frq)[0][0]
...
srt_ids = np.argsort(transs[t_frqs_idx,5,0])

intp_field = np.interp(t_frq, transs[t_frqs_idx[srt_ids],5,0], B_vec[t_frqs_idx[srt_ids]])
...
# also get gamma 23 at that resonance field, as this is important to get B1 from the experimental nu1 fit
# remembering: transition frequency in transs[:,5,0], transition moment from the excitation hamiltonian in transs[:,5,2]

intp_gamma = np.interp(t_frq, transs[t_frqs_idx[srt_ids],5,0], transs[t_frqs_idx[srt_ids],5,2])


print('... and accompanying \gamma_23 value in MHz/mT: {:.03f}'.format(intp_gamma))
